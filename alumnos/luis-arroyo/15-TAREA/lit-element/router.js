...
const getPokemons = async (limit, offset) => {
  ...
  const promisesArray = data.results.map((item) => getPokemon(item.url));
...
async function loadHomeContent() {
  ...
  if (!grid) {
    console.error(
      "Error: El componente <grid-element> no se encontró en el DOM."
    );
    return;
  }

  try {
    const initialResults = await getPokemons(limit, 0);
    grid.pokemons = initialResults;
    offset = 0;
    if (loadButton) {
      loadButton.addEventListener("click", () => {
        ...
        getPokemons(limit, offset)
          .then((newResults) => {
            const currentPokemons = grid.pokemons || [];
            grid.pokemons = [...currentPokemons, ...newResults];
    ...
    console.error("Fallo al cargar contenido de Home:", error);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const match = currentFullPath.match(/.*\/15-TAREA\/?/);
  const BASE_PATH = match ? match[0].replace(/\/$/, "") : "";

  const pathVirtual = currentFullPath.replace(BASE_PATH, "");
  const initialPath = pathVirtual === "" ? "/login" : pathVirtual;
