...
//Funcion para agrupar por el primaryJobCompany
function groupByPrimaryCompany(people) {
    return groupBy(people, p => {
        if (p.jobs.length === 0) return "unemployed";
        const bestJob = p.jobs.reduce((max, j) => j.salary > max.salary ? j : max);
        return bestJob.company;
    });
}

console.log(groupByPrimaryCompany(people))
