...
//Funcion compose para filterAdults, mapToSummary y sortByName
const compose = (...funcs) => input =>
    funcs.reduceRight((acc, fn) => fn(acc), input);

const filterAdults = people => people.filter(p => p.age >= 18);
const mapToSummary = people => people.map(p => ({
    id: p.id,
    name: p.name,
    city: p.address.city
}));
const sortByName = people => [...people].sort((a, b) => a.name.localeCompare(b.name));
