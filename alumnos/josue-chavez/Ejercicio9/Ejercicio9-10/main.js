...
//Funcion para mostrar las personas con más de un trabajo y mostrando el salario promedio entre sus trabajos
function multiJobAverages(people) {
    validateArray(people)
    return people
        .filter(p => p.jobs.length > 1)
        .map(p => {
        const avgSalary = p.jobs.reduce((acc, j) => acc + j.salary, 0) / p.jobs.length;
        return { id: p.id, name: p.name, avgSalary };
        });
}
