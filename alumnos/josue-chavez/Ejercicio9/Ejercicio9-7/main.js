...
//Funcion para encontrar la skill más común entre todo el arreglo
function mostCommonSkill(people) {
    ...
    return Object.entries(counts).reduce((max, [skill, count]) =>
        count > max.count ? { skill, count } : max, { skill: null, count: 0 }
    ).skill;
}
