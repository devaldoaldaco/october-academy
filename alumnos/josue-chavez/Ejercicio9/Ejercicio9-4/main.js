...
//Funcion que devuelve todas las personas con familia que tiene menos de la edad enviada por parametro
function withMinors(people, edad) {
    validateArray(people)
    const peopleWithMinors = people.filter(person => {
        return person.family?.some(member => member.age < edad);
    });

    return peopleWithMinors;
}

console.log(withMinors(people,13))
