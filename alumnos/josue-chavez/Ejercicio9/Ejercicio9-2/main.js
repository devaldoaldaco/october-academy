...
//Trae todas las personas que pertenecen a la ciudad filtrada
function findByCity(people, city){

    validationArray(people)

    const personsFilter = people.filter(
        person => person.address.city === city
    )
    
    const personsMap = personsFilter.map(peopleToPerson)

    const personsAgeAsc = ageDesc(personsMap)

    return personsAgeAsc
}

//Mapeo de people a formato person con name y age
function peopleToPerson(persons) {
    return {
        name: persons.name,
        age: persons.age,
    };
