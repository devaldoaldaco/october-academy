...
const tesla = new ElectricCar('Tesla','Model 3',2021,4,75);
const tesla2 = new Vehicle('Tesla','Model 3',2021);
console.log(tesla.info());
console.log(tesla2.info());
...
class Library{
    ...
    addBook(book){
        this.books.map(book.id,book);
    }
}

class Book{
    constructor(id,nombre,autor){
        this.id = id;
        this.nombre = nombre;
        this.autor = autor;
    }
...
class BankAccount{
    ...
    toString(){
        return `Cuenta de ${this.owner}: $${this.balance}`;
    }
...
console.log(BankAccount.prototype.hasOwnProperty('#balance')); // false
...
const r = new Range(1,3);
Range.prototype.min = 1;
Range.prototype.max = 3;
console.log(2 instanceof Range); // true
console.log(5 instanceof Range); // false
console.log(r instanceof Range); // true
...
class SuperAgent{
    ...
    walk(){
        if(this._walker){
            ...
            throw new Error("No puedo caminar");
        }
    }
}
...
function serialize(obj){
    return JSON.stringify(obj, (key, value) => {
        if(value && classRegistry.has(value.constructor.name)){
            return {
...
function deserialize(jsonStr){
    return JSON.parse(jsonStr, (key, value) => {
        if(value && value.__class__ && classRegistry.has(value.__class__)){
            const ctor = classRegistry.get(value.__class__);
...
registerClass('Person', class Person{
    constructor(name){
        this.name = name;
        this.age = 0;
    }
...
const s = serialize({ owner: p });
const restored = deserialize(s);
console.log(restored.owner instanceof Person); // true
