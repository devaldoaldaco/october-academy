...
//Funcion para sumar el total de salario por persona
function sumSalaries(person) {
    if (person.jobs && person.jobs.length > 0) {
        
        const totalSalary = person.jobs.reduce((sum, job) => {
            return sum + job.salary;
        }, 0);
...
function totalSalariesAll(people){
    ...
    const personsSalaryTotal = people.map(
        (person) => {
            const salaryTotal = sumSalaries(person)
            return peopleToPerson(person,salaryTotal)
        }
    )
    
    const personsSalaryTotalDesc = salaryDesc(personsSalaryTotal)
    return personsSalaryTotalDesc
}

//Mapeo de people a formato person con id, name y totalSalary
function peopleToPerson(persons, totalSalary) {
    return {
        id: persons.id,
        name: persons.name,
...
function salaryDesc(persons){
    return persons.sort((a, b) => b.totalSalary - a.totalSalary);
}
