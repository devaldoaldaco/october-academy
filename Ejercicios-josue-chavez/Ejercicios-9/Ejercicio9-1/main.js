...
Tarea: crear getContacts(people) que devuelva un arreglo plano con 
objetos { id, name, email, city, primaryJob }, donde primaryJob es 
el title del job con mayor salary (si no hay jobs → null).
Salida esperada (parcial): { id:1, name:"Ana Ruiz", email:"ana.ruiz@example.com", 
...
//Funcion para retornar arreglo plano por cada persona
function peopleToPerson(person) {
    let primaryJob = maxSalaryForJobs(person)
    return {
        id: person.id,
        name: person.name,
        email: person.email,
...
//Funcion para sacar el Trabajo con mayor salario
function maxSalaryForJobs(person){
    let primaryJob = null;
    if (person.jobs && person.jobs.length > 0) {
        const bestJob = person.jobs.reduce((max, job) => job.salary > max.salary ? job : max, person.jobs[0]);
        primaryJob = bestJob.title;
    }
    return primaryJob
}

//Devuelve undefined porque no estoy retornando nada dentro del map
// function getContacts(people) {
//     return people.map(
//         (person) => {
//             peopleToPerson(person)
//         }
//     );
// }

//Funcion corregida poniendo return dentro del map
//Funcion para devolver arreglo plano a partir del arreglo de people
function getContacts(people) {
    validationArray(people)
    return people.map(
        (person) => {
            return peopleToPerson(person)
        }
    );
}

const arrayPlano = getContacts(people);
