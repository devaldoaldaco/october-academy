...
//Funcion para mapear cada objeto a id,name y estado
function peopleToPerson(people){
...
//Funcion para agrupar por estados
function groupBy(people){
    return people.reduce((acc, person) => {
        const key = person.state;
        if (!acc[key]) acc[key] = [];
        acc[key].push(person);
        return acc;
    }, {});
...
//Funcion que devuelve un arreglo con todos los estados y las personas que pertenecen a cada estado
function groupByState(people) {
    validateArray(people)
    const arrayPeopleState = peopleToPerson(people)

    const grouped = groupBy(arrayPeopleState)
