...
//Funcion para unir transacciones con el mismo id y sumar sus montos
function mergeTransactions(person) {
    const merged = Object.values(
        person.transactions.reduce((acc, t) => {
        if (!acc[t.id]) acc[t.id] = { ...t };
        else acc[t.id].amount += t.amount;
        return acc;
        }, {})
    );
    return { ...person, transactions: merged };
}
